...
int main(){
   int fila[TAM], opcao, index = -1, x; // x é usada na iteração dos laços for
      ...
      switch(opcao){
         case 1:
            if(index == TAM-1){ // verifica se a pilha não está cheia antes que o usuario digite o elemeto
               ...
               scanf("%d", &fila[index]); // o elemento digitado é atribuido diretamente a fila
            }
            break;
         ...
         case 3:
            if(index < -1){ // verifica o index para saber se a fila está vazia ou não, se não estiver vazia a fila é percorrida
               printf("\nA PILHA ESTA VAZIA\n");
            }else{
               printf("\nFILA ATUAL: => ");
